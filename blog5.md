...
Then applied gamma correction (pow(color, 1.0/gamma)) before quantizing to [0, 255]. This two-stage process preserves hue while controlling saturation in the final output.
...
Why quartic (exponent = 4)? I tested various exponents—linear (exponent 1) looked too harsh, quadratic (2) was better, but quartic gave the smoothest visual falloff while maintaining a sharp outer edge. The formula:

Code
s = [(cos(α) - cos(θ_c/2)) / (cos(θ_f/2) - cos(θ_c/2))]^4
...
This concentrates samples near the normal direction—perfect for importance sampling Lambertian BRDFs. The trick: cos(θ) = √ξ₂ directly, which I derived by inverting the solid angle CDF weighted by cos(θ).
...
Key Insight: With cosine-weighted sampling, the PDF exactly matches the cos(θ) term in the integrand, causing perfect cancellation. This is why importance sampling is so powerful—it reduces variance to zero for perfectly diffuse surfaces with uniform environment lighting.
